...
<body>
            ...
            <div class="card">
                ...
                <div class="transaction-list" id="transactionHistory">
                    {{-- <div class="transaction-item">
                        ...
                        <p></p>
                        <p></p>
                        <span class="status-badge status-process"></span>
                    </div>
                    <div class="transaction-item">
                        <h4></h4>
                        <p></p>
    ...
    <script>
        ...
        function addService(serviceName, price, serviceId) {
            const serviceSelect = document.getElementById('serviceType');

            // Cari option berdasarkan value (id)
            for (let i = 0; i < serviceSelect.options.length; i++) {
                if (serviceSelect.options[i].value == serviceId) {
                    serviceSelect.selectedIndex = i;
                    // update data-name dan data-price untuk addToCart
                    updatePrice();
                    break;
                }
            }
        ...
        function updatePrice() {
            ...
            const serviceName = selected.getAttribute('data-name');
            const servicePrice = parseInt(selected.getAttribute('data-price'));

            // Simpan data sementara supaya addToCart bisa pakai
            select.dataset.selectedName = serviceName;
            select.dataset.selectedPrice = servicePrice;
        }
        ...
        function addToCart() {
            ...
            if (!weight || !serviceSelect.value) {
                alert("Pilih layanan dan masukkan berat/jumlah!");
                return;
            }

            const serviceName = serviceSelect.dataset.selectedName;
            const servicePrice = parseInt(serviceSelect.dataset.selectedPrice);
            const idService = serviceSelect.value;

            const subtotal = weight * servicePrice;

            cart.push({
                // id: cartId++, // buat hapus item
        ...
        async function processTransaction() {
            ...
            const customerName = customerSelect.options[customerSelect.selectedIndex].text; // nama string
            const customerPhone = customerSelect.options[customerSelect.selectedIndex].getAttribute('data-phone') ||
                '';
            const customerAddress = customerSelect.options[customerSelect.selectedIndex].getAttribute(
                    'data-address') ||
                '';
            ...
            const transaction = {
                id_customer: customerId, // dari select
                order_code: `TRX-${transactionCounter.toString().padStart(3, '0')}`,
                ...
                services: cart.map(item => ({
                    id_service: item.id_service, // pastikan item punya ini
                    qty: item.weight,
                    price: item.price,
            ...
            const receiptHtml = `
                <div class="receipt">
                    <div class="receipt-header">
                        ...
                        <p>ID: ${transaction.customer.name}</p>
        ...
        function checkout() {
            ...
            document.getElementById('receiptCustomer').textContent =
                `Customer: ${document.getElementById('customerName').options[document.getElementById('customerName').selectedIndex].text}`;
            ...
            document.getElementById('receiptTotal').textContent = totalAmount.toLocaleString();
        }
        ...
        function updateTransactionHistory() {
            ...
            const html = recentTransactions.map(transaction => `
                <div class="transaction-item">
                    <h4>${transaction.order_code} - ${transaction.customer.name}</h4>
